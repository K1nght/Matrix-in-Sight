...
class Trainer(object):
    def __init__(self, weight_path, resume, gpu_id, accumulate, save_path, resume_path=None, resume_n_epochs=0):
        init_seeds(0)
        ...
        self.resume_n_epochs = resume_n_epochs
    ...
    def __load_resume_weights(self):
        ...
        print("Resume the training process")
        last_weight = os.path.join(self.resume_path, "last.pt")
        chkpt = torch.load(last_weight, map_location=self.device)
        self.yolov4.load_state_dict(chkpt["model"])
        print("Success in loading the last weight at %s" % last_weight)

        self.start_epoch = chkpt["epoch"] + 1
        self.epochs = self.start_epoch + self.resume_n_epochs
    ...
    def __save_model_weights(self, epoch, mAP):
        ...
        if epoch > 0 and epoch % 5 == 0:
            ...
            torch.save(chkpt["model"], os.path.join(
                            self.save_path,
                            "epoch%g.pt" % epoch,
                        ),)
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (
                ...
                if i % 10 == 0:
                    ...
                    logger.info(
                        "  === Epoch:[{:3}/{}],step:[{:3}/{}],img_size:[{:3}],total_loss:{:.4f}|loss_ciou:{:.4f}|loss_conf:{:.4f}|loss_cls:{:.4f}|lr:{:.4e}".format(
                            epoch+1,
                            self.epochs,
                            i,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weight_path",
        type=str,
        default="weights/mobilenetv3.pth",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="where to resume the training process",
    )
    parser.add_argument(
        "--resume_n_epochs",
        type=int,
        default=0,
        help="how many epochs continue to train",
    )
    parser.add_argument(
        "--gpu_id",
        type=int,
    ...
    parser.add_argument("--save_dir", type=str, default="checkpoint", help="save dirctory")
    parser.add_argument(
        "--accumulate",
        type=int,
        default=4,
    ...
    if opt.mnist:
        ...
        import config.config_CTC as cfg

    save_path = os.path.join(opt.save_dir, time.strftime("%b-%d[%H.%M.%S]-", time.localtime()) + cfg.MODEL_TYPE["TYPE"])
    ...
    Trainer(
        ...
        resume_n_epochs=opt.resume_n_epochs,
        gpu_id=opt.gpu_id,
