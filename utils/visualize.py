...
def visualize_boxes(image, boxes, labels, probs, class_labels, txt_path):
    ...
    image = visualize_boxes_and_labels_on_image_array(
        image, boxes, labels, probs, category_index, txt_path=txt_path,
    )
    return image
    ...
    for i in range(min(max_boxes_to_draw, boxes.shape[0])):
        if scores is None or scores[i] > min_score_thresh:
            ...
            if scores is None:
                ...
                xmin, ymin, xmax, ymax = str(round(xmin)), str(round(ymin)), str(round(xmax)), str(round(ymax))
                box = [x for x in box]
                oneline = [str(i), str(classes[i]), class_name, xmin, ymin, xmax, ymax, "%.4f" % scores[i]]
                with open(txt_path, "a") as f:
                    f.write(" ".join(oneline) + "\n")
