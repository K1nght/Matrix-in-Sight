...
def img_proce(img, xmin, ymin, xmax, ymax, model, mnist=True, width_new=28, height_new=28):
    ...
    pre_number = np.argmax(model.predict(tens_img).detach().numpy())

    return pre_number

if __name__ == '__main__':

    img_path = 'det_result/det-12-27/115/115.jpg'
    txt_path = 'det_result/det-12-27/115/115.txt'

    txtFile = open(txt_path)
    txtList = txtFile.readlines()
    img = cv2.imread(img_path)

    model_path = 'checkpoint/mnist-last1.pth'
    ...
    for oneline in txtList:
        ...
        ymax = ymax+y_pad
        if 'number' in label:
            pre = img_proce(img, xmin, ymin, xmax, ymax, model=model, width_new=28,)
            print(pre)
