...
class Evaluator(object):
    ...
    def APs_voc(self, multi_test=False, flip_test=False):
        val_data_expr = os.path.join(
            self.val_data_path, "val", "*.jpg"
        )

        val_data_paths = glob.glob(val_data_expr)
        ...
        for val_data_path in tqdm(val_data_paths):
            ...
            f.close()
        self.inference_time = 1.0 * self.inference_time / len(val_data_paths)
